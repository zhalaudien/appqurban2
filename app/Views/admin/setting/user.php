        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <small class="text-muted">Pengaturan User</small>
                </div>
                        ...
                        <table id="datatablesSimple" class="table table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                foreach ($user as $s) : ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td class="fw-semibold"><?= esc($s['username']) ?></td>
                                        <td class="fw-semibold"><?= esc($s['nama']) ?></td>
                                        <td class="fw-semibold"><?= esc($s['role_id']) ?></td>
                                        <td class="fw-semibold"><?= esc($s['nama_cabang']) ?></td>
                                        <td class="fw-semibold"><?= $s['pusat'] ?></td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modalEdit<?= $s['id'] ?>">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <a href="/setting/user/delete/<?= $s['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Hapus user ini?')">
                                    ...
                                    <div class="modal fade" id="modalEdit<?= $s['id'] ?>" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form action="/setting/user/update/<?= $s['id'] ?>" method="post">
                                                    <?= csrf_field() ?>
                                                    <div class="modal-body">
                                                        <div class="mb-3">
                                                            <label class="form-label">Username</label>
                                                            <input type="text" name="username" class="form-control" value="<?= esc($s['username']) ?>" required>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label class="form-label">Nama</label>
                                                            <input type="text" name="nama" class="form-control" value="<?= esc($s['nama']) ?>" required>
                                                        </div>
                                                        <div class="mb-3">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <select name="role_id" class="form-select" required>
                                                                ...
                                                                <?php foreach ($role as $c): ?>
                                                                    <option value="<?= $c['id']; ?>" <?= (int)$s['role_id'] === (int)$c['id'] ? 'selected' : ''; ?>>
                                                                        <?= esc($c['role_name']); ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <select name="cabang_id" class="form-select" required>
                                                                ...
                                                                <?php foreach ($cabang as $c): ?>
                                                                    <option value="<?= $c['id']; ?>" <?= (int)$s['cabang_id'] === (int)$c['id'] ? 'selected' : ''; ?>>
                                                                        <?= esc($c['nama_cabang']); ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="text" name="pusat" class="form-control" value="<?= esc($s['pusat']) ?>" required>
                                                        </div>
                                                    </div>
    ...
    <div class="modal fade" id="modalTambah" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="/setting/user/create" method="post">
                    <?= csrf_field() ?>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Username</label>
